...
REPORT="$HOME/Desktop/OCLP7_D97AEN_GITHUB_ARTIFACT_EXACT_DOWNLOAD_AUDIT_AND_DEPLOY_REPORT.txt"
...
STAGE="$HOME/Desktop/OCLP7_D97AEN_GITHUB_ARTIFACT_STAGE_$STAMP"
...
echo "===== OCLP7 D97AEN — EXACT GITHUB ARTIFACT DOWNLOAD / AUDIT / DEPLOY ====="
...
for tool in gh git python3 shasum awk find ditto lipo file plutil pgrep ps open sudo df; do
...
print('D97AEN_WORKFLOW_RUN_IDENTITY_AND_SUCCESS=PASS')
...
print('D97AEN_ARTIFACT_METADATA_IDENTITY=PASS')
PY
...
echo "D97AEN_INNER_ZIP_AND_SUM_MANIFEST=PASS"
...
echo "D97AEN_DOWNLOADED_BUILD_AUDIT_REPORT=PASS"
...
echo "D97AEN_STAGED_APP_IDENTITY=PASS"
...
echo "D97AEN_LIVE_D97Z_PREIMAGE=PASS"
...
for pid in $PIDS_BEFORE; do
  ...
  echo "TERMINATING_OLD_PROCESS=$pid|$cmd"
...
if [[ "$LIVE_POST_SHA" != "$D97AD_EXE_SHA_EXPECTED" ]]; then
  echo "D97AEN_DEPLOY_POST_SHA_MISMATCH=$LIVE_POST_SHA"
  sudo /bin/rm -rf "$LIVE_APP"
  sudo /usr/bin/ditto "$BACKUP_APP" "$LIVE_APP"
  RESTORED_SHA="$(sha256_file "$LIVE_EXE")"
  echo "D97AEN_ROLLBACK_RESTORED_SHA=$RESTORED_SHA"
  fail "DEPLOY_SHA_MISMATCH_ROLLBACK_ATTEMPTED"
fi
echo "D97AEN_DEPLOY_SHA_MATCH=PASS"
...
for pid in $PIDS_AFTER; do
  [[ -n "$pid" ]] || continue
  cmd="$(/bin/ps -p "$pid" -o command= 2>/dev/null || true)"
  echo "LIVE_PROCESS=$pid|$cmd"
  if [[ "$cmd" == /Applications/OpenCore-Patcher.app/Contents/MacOS/OpenCore-Patcher* ]]; then
    PROVEN=1
  fi
done
(( PROVEN == 1 )) || fail "FRESH_PROCESS_PATH_PROVENANCE_FAIL"
echo "D97AEN_FRESH_PROCESS_PROVENANCE=PASS"

echo "===== FINAL ====="
echo "D97AEN_WORKFLOW_RUN=PASS"
echo "D97AEN_ARTIFACT_METADATA=PASS"
echo "D97AEN_DOWNLOADED_ARTIFACT_AUDIT=PASS"
echo "D97AEN_STAGED_APP_EXE_SHA256=$STAGED_EXE_SHA"
echo "D97AEN_BACKUP_APP=$BACKUP_APP"
echo "D97AEN_LIVE_D97AD_EXE_SHA256=$LIVE_POST_SHA"
echo "D97AEN_GITHUB_ARTIFACT_EXACT_DOWNLOAD_AUDIT_AND_DEPLOY=PASS"
echo "LOCAL_SOURCE_MUTATION=NO"
echo "ROOT_PATCH=AUTO-NO"
echo "REBOOT=AUTO-NO"
