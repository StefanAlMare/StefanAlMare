...
# OCLP7 D97GD — corrected read-only audit of just-patched underlying APFS System volume.
# Fixes D97GC parsing of snapshot devices such as disk1s8s1 -> underlying System volume disk1s8.
# NO Root Patch. NO Restore. NO system-volume writes. NO EFI/NVRAM/framebuffer changes. NO reboot.
...
OUT="$HOME/Desktop/OCLP7_D97GD_POSTPATCH_SYSTEM_VOLUME_AUDIT_${STAMP}"
REPORT="$OUT/D97GD_REPORT.txt"
TSV="$OUT/D97GD_METALLIBS.tsv"
MNT="/private/tmp/OCLP7_D97GD_SYSVOL_${STAMP}"
ZIP="$HOME/Desktop/OCLP7_D97GD_POSTPATCH_SYSTEM_VOLUME_AUDIT_${STAMP}.zip"
...
fail() {
    echo "D97GD_STATUS=FAIL"
    echo "D97GD_REASON=$*"
    echo "D97GD_REPORT=$REPORT"
    exit 1
}
...
normalize_apfs_snapshot_device() {
    local dev="$1"
    if [[ "$dev" =~ ^disk[0-9]+s[0-9]+s[0-9]+$ ]]; then
        printf '%s\n' "$dev" | /usr/bin/sed -E 's/^(disk[0-9]+s[0-9]+)s[0-9]+$/\1/'
        return 0
    fi
    if [[ "$dev" =~ ^disk[0-9]+s[0-9]+$ ]]; then
        printf '%s\n' "$dev"
        return 0
    fi
    return 1
...
echo "===== D97GD POST-PATCH SYSTEM-VOLUME AUDIT V2 ====="
echo "ROOT_PATCH=NO"
echo "RESTORE=NO"
...
print(f'D97GD_LOCAL_METALLIB_COUNT={len(m)}')
print('D97GD_LOCAL_ALL_MTLB=PASS')
print('D97GD_LOCAL_CORE_SHA=PASS')
PY

echo
echo "===== LOCATE UNDERLYING APFS SYSTEM VOLUME ====="
SNAPS="$(/usr/sbin/diskutil apfs listSnapshots / 2>&1 || true)"
printf '%s\n' "$SNAPS" > "$OUT/D97GD_SNAPSHOTS.txt"
RAWDEV="$(printf '%s\n' "$SNAPS" | /usr/bin/sed -n 's/^Snapshots for \(disk[^ ]*\).*/\1/p' | /usr/bin/head -n 1)"

if [[ -z "$RAWDEV" ]]; then
    RAWDEV="$(/usr/sbin/diskutil info / | /usr/bin/awk '/Device Identifier:/ {print $3; exit}')"
fi
[[ -n "$RAWDEV" ]] || fail "ROOT_DEVICE_NOT_FOUND"
echo "D97GD_RAW_ROOT_OR_SNAPSHOT_DEVICE=$RAWDEV"

SYSVOL="$(normalize_apfs_snapshot_device "$RAWDEV" || true)"
[[ -n "$SYSVOL" ]] || fail "SYSTEM_VOLUME_NORMALIZATION_FAILED_$RAWDEV"
echo "D97GD_SYSTEM_VOLUME=$SYSVOL"
[[ "$SYSVOL" =~ ^disk[0-9]+s[0-9]+$ ]] || fail "SYSTEM_VOLUME_PARSE_FAILED_$SYSVOL"
[[ -e "/dev/$SYSVOL" ]] || fail "SYSTEM_VOLUME_DEVICE_MISSING"

/usr/sbin/diskutil info "$SYSVOL" > "$OUT/D97GD_SYSTEM_VOLUME_INFO.txt" 2>&1 || fail "SYSTEM_VOLUME_INFO_FAILED"
if ! /usr/bin/grep -Eiq 'APFS Volume.*\(System\)|Role:.*System|Volume Name:.*macOS|Mounted:.*Yes|Mounted:.*No' "$OUT/D97GD_SYSTEM_VOLUME_INFO.txt"; then
    echo "===== SYSTEM VOLUME INFO ====="
    /bin/cat "$OUT/D97GD_SYSTEM_VOLUME_INFO.txt"
    fail "SYSTEM_VOLUME_ROLE_SANITY_FAILED"
fi
echo "D97GD_SYSTEM_VOLUME_NORMALIZATION=PASS"
...
echo "D97GD_READONLY_MOUNT=PASS"
...
print(f'D97GD_PATCHED_METALLIB_EXACT={exact}')
print(f'D97GD_PATCHED_METALLIB_MISSING={missing}')
print(f'D97GD_PATCHED_METALLIB_DIFFERENT={different}')
print(f'D97GD_PATCHED_METADATA_STUB={stub}')
if bad:
    print('D97GD_MISMATCH_SAMPLES='+';'.join(f'{p}:{s}' for p,s in bad[:20]))
...
print(f'D97GD_PATCHED_CORE_BYTES={len(cb)}')
print(f'D97GD_PATCHED_CORE_SHA256={csha}')
print(f'D97GD_PATCHED_CORE_MAGIC={cb[:4].hex()}')
...
print('D97GD_PATCHED_CORE_IDENTITY=PASS')
print('D97GD_180_DYNAMIC_METALLIB_IDENTITY=PASS')
PY
...
echo "D97GD_HASWELL_DRIVER_PRESENCE=PASS"
...
echo "D97GD_OFFICIAL_HELPER_IDENTITY=PASS"
...
MOUNTED=0
/bin/rmdir "$MNT" || true
echo "D97GD_AUDIT_UNMOUNT=PASS"

echo
echo "===== PACKAGE EVIDENCE ====="
/usr/bin/ditto -c -k --sequesterRsrc --keepParent "$OUT" "$ZIP"
echo "D97GD_ZIP=$ZIP"
echo "D97GD_ZIP_SHA256=$(sha256 "$ZIP")"
echo "D97GD_ZIP_BYTES=$(/usr/bin/stat -f '%z' "$ZIP")"

echo
echo "===== FINAL ====="
echo "D97GD_STATUS=PASS"
echo "D97GD_PATCHED_SYSTEM_VOLUME=PASS"
echo "D97GD_180_DYNAMIC_METALLIB_IDENTITY=PASS"
echo "D97GD_PATCHED_CORE_IDENTITY=PASS"
echo "D97GD_HASWELL_DRIVER_PRESENCE=PASS"
echo "D97GD_OFFICIAL_HELPER_IDENTITY=PASS"
echo "D97GD_REBOOT=NO"
echo "D97GD_REPORT=$REPORT"
