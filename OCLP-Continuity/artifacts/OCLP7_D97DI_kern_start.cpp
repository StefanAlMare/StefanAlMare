//
// OCLPMetalCompat.kext — D97DI latent functional D97BV prototype
//
// Purpose:
// Preserve the runtime-proven D97DD _cs_validate_page delivery path and add the
// exact D97BV site+cave mutations behind an explicit second boot argument.
//
// Safety:
...
static void recordPassOrFirstNegative(_Atomic(uint32_t) *state, bool pass) {
    if (pass) {
        atomic_store_explicit(state, 1U, memory_order_relaxed);
