...
# OCLP7 D97GK — fixed read-only evidence capture for the two corrected-Root-Patch accelerated boots.
# Fixes D97GJ's pipefail/SIGPIPE stop at `last reboot | head`.
# User-authoritative boot boundaries:
#   accelerated #1: reboot 2026-09-07 23:41 local, ends before 23:50 reboot
...
OUT="$HOME/Desktop/OCLP7_D97GK_TWO_ACCEL_BOOT_LOG_CAPTURE_${STAMP}"
ZIP="$HOME/Desktop/OCLP7_D97GK_TWO_ACCEL_BOOT_LOG_CAPTURE_${STAMP}.zip"
mkdir -p "$OUT/diagnostics" "$OUT/d97ew"

exec > >(tee "$OUT/D97GK_CAPTURE.txt") 2>&1

echo "===== D97GK TWO ACCELERATED BOOT LOG CAPTURE V2 ====="
echo "ROOT_PATCH=NO"
echo "RESTORE=NO"
...
capture_window "ACCEL1_2341" "2026-09-07 23:41:00" "2026-09-07 23:49:59"
...
print(f'D97GK_DIAGNOSTIC_REPORT_COUNT={ok}')
for r in rows:
    print('D97GK_DIAGNOSTIC='+r[0])
PY
...
echo "D97GK_ZIP=$ZIP"
echo "D97GK_ZIP_SHA256=$ZIP_SHA"
echo "D97GK_ZIP_BYTES=$ZIP_BYTES"
echo "D97GK_STATUS=CAPTURE_COMPLETE"
echo "D97GK_REBOOT=NO"
