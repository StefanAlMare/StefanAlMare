...
# OCLP7 D97GQ — reconstruct historical P1 selector bridge on a disposable copy only.
...
OUT="$HOME/Desktop/OCLP7_D97GQ_P1_RECONSTRUCTION_${STAMP}"
COPY="$OUT/MTLCompilerService.P1.copy"
REPORT="$OUT/D97GQ_REPORT.txt"
ZIP="$HOME/Desktop/OCLP7_D97GQ_P1_RECONSTRUCTION_${STAMP}.zip"
mkdir -p "$OUT"
exec > >(tee "$REPORT") 2>&1

fail(){ echo "D97GQ_STATUS=FAIL"; echo "D97GQ_REASON=$*"; echo "D97GQ_REPORT=$REPORT"; exit 1; }
sha256(){ /usr/bin/shasum -a 256 "$1" | /usr/bin/awk '{print $1}'; }

cat <<'HDR'
===== D97GQ READ-ONLY P1 SELECTOR RECONSTRUCTION =====
SYSTEM_MUTATION=NO
ROOT_PATCH=NO
RESTORE=NO
EFI_MUTATION=NO
...
print(f'D97GQ_PREIMAGE_OFFSET=0x{off:x}')
print(f'D97GQ_PREIMAGE_HEX={actual.hex()}')
if actual != pre:
    raise SystemExit('PREIMAGE_MISMATCH')
# prove unique exact preimage occurrence before patching
occ=[]; start=0
while True:
    i=bytes(b).find(pre,start)
    if i<0: break
    occ.append(i); start=i+1
print('D97GQ_PREIMAGE_OCCURRENCES=' + ','.join(hex(x) for x in occ))
print(f'D97GQ_PREIMAGE_OCCURRENCE_COUNT={len(occ)}')
if len(occ)!=1 or occ[0]!=off:
    raise SystemExit('PREIMAGE_NOT_UNIQUE_AT_EXPECTED_OFFSET')
b[off:off+len(pre)]=post
p.write_bytes(b)
print(f'D97GQ_POSTIMAGE_HEX={bytes(b[off:off+len(post)]).hex()}')
PY

echo
echo "===== PATCHED COPY IDENTITY ====="
PATCHED_SHA="$(sha256 "$COPY")"
echo "D97GQ_PATCHED_COPY_SHA256=$PATCHED_SHA"
if [[ "$PATCHED_SHA" == "$EXPECTED_P1_SHA" ]]; then
  echo "D97GQ_PATCHED_COPY_MATCHES_HISTORICAL_P1=PASS"
else
  echo "D97GQ_PATCHED_COPY_MATCHES_HISTORICAL_P1=FAIL"
fi

# Byte-level delta proof: exactly bytes within the 6-byte selector instruction may differ.
...
print(f'D97GQ_DIFF_BYTE_COUNT={len(d)}')
print('D97GQ_DIFF_OFFSETS=' + ','.join(hex(i) for i in d))
for i in d: print(f'D97GQ_DIFF_{i:#x}={A[i]:02x}->{B[i]:02x}')
...
echo "===== RESULT ====="
if [[ "$PATCHED_SHA" == "$EXPECTED_P1_SHA" ]]; then
  echo "D97GQ_P1_RECONSTRUCTION=EXACT_HISTORICAL_MATCH"
  echo "D97GQ_CLASSIFICATION=STATIC_STRUCTURAL_SEMANTIC_PROVEN"
else
  echo "D97GQ_P1_RECONSTRUCTION=BYTE_PATCH_VALID_BUT_HISTORICAL_SHA_MISMATCH"
  echo "D97GQ_CLASSIFICATION=STATIC_PATCH_SEMANTICS_PROVEN_IDENTITY_NOT_CLOSED"
fi

/usr/bin/ditto -c -k --sequesterRsrc --keepParent "$OUT" "$ZIP"
echo "D97GQ_ZIP=$ZIP"
echo "D97GQ_ZIP_SHA256=$(sha256 "$ZIP")"
echo "D97GQ_ZIP_BYTES=$(/usr/bin/stat -f '%z' "$ZIP")"
echo "D97GQ_STATUS=PASS_READONLY_COPY_RECONSTRUCTION"
echo "D97GQ_REBOOT=NO"
