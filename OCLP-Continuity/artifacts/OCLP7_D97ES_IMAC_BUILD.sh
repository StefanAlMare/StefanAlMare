...
# OCLP7 D97ES — Intel iMac local build only.
# Live-readable IORegistry tuple telemetry successor to D97EL.
# No EFI / Root Patch / NVRAM / reboot / target mutation.

STAMP="$(date +%Y%m%d_%H%M%S)"
DESKTOP="${HOME}/Desktop"
WORK="${DESKTOP}/OCLP7_D97ES_BUILD_${STAMP}"
...
REPORT="${PACKAGE}/D97ES_BUILD_REPORT.txt"
LOG="${PACKAGE}/D97ES_XCODEBUILD.log"
ZIP="${DESKTOP}/OCLP7_D97ES_IMAC_BUILD_${STAMP}.zip"
...
EL_SOURCE_SHA="3a103b84ae8c18c7259c672dff9dd5c0f11d04d88554c8da8e60c5c0c04b9003"
...
die(){ log "D97ES_BUILD_STATUS=FAIL"; log "FAIL_REASON=$*"; exit 1; }

log "===== D97ES BUILD ====="
log "HOST_REQUIRED=INTEL_IMAC"
log "PLUGIN_VERSION=0.0.11"
log "D97BV_PRESERVED=YES"
log "D97EH_OBSERVER_SEMANTICS_PRESERVED=YES"
log "D97EL_ROUTE_TELEMETRY_PRESERVED=YES"
log "LIVE_IOREG_TUPLE_TELEMETRY=YES"
log "CAPTURE_SLOTS=8"
...
log "FILESYSTEM_TELEMETRY=NO"
log "NVRAM_TELEMETRY=NO"
...
/usr/bin/git -C "${PROJECT}" show "${ES_GEN_COMMIT}:${ES_GEN_PATH}" > "${WORK}/D97ES_generator.py" || die "D97ES_generator_fetch_failed"
...
EL_BLOB_ACTUAL="$(/usr/bin/git -C "${PROJECT}" rev-parse "${EL_GEN_COMMIT}:${EL_GEN_PATH}" 2>/dev/null || true)"
[[ "${EL_BLOB_ACTUAL}" == "${EL_GEN_BLOB}" ]] || die "D97EL_generator_blob_mismatch"
ES_BLOB_ACTUAL="$(/usr/bin/git -C "${PROJECT}" rev-parse "${ES_GEN_COMMIT}:${ES_GEN_PATH}" 2>/dev/null || true)"
[[ "${ES_BLOB_ACTUAL}" == "${ES_GEN_BLOB}" ]] || die "D97ES_generator_blob_mismatch"
log "D97EL_GENERATOR_BLOB=${EL_BLOB_ACTUAL}"
log "D97ES_GENERATOR_COMMIT=${ES_GEN_COMMIT}"
log "D97ES_GENERATOR_BLOB=${ES_BLOB_ACTUAL}"
log "D97ES_GENERATOR_SHA256=$(/usr/bin/shasum -a 256 "${WORK}/D97ES_generator.py" | awk '{print $1}')"
...
[[ "$(/usr/bin/shasum -a 256 "${WORK}/D97EL.cpp" | awk '{print $1}')" == "${EL_SOURCE_SHA}" ]] || die "D97EL_source_sha_mismatch"

"${PY}" "${WORK}/D97ES_generator.py" "${WORK}/D97EL.cpp" "${WORK}/D97ES.cpp" | tee -a "${REPORT}"
"${PY}" "${WORK}/D97ES_generator.py" "${WORK}/D97EL.cpp" "${WORK}/D97ES.repeat.cpp" >/dev/null
SRC="${WORK}/D97ES.cpp"
SRC_REPEAT="${WORK}/D97ES.repeat.cpp"
SRC_SHA="$(/usr/bin/shasum -a 256 "${SRC}" | awk '{print $1}')"
SRC_REPEAT_SHA="$(/usr/bin/shasum -a 256 "${SRC_REPEAT}" | awk '{print $1}')"
[[ "${SRC_SHA}" == "${SRC_REPEAT_SHA}" ]] || die "D97ES_nondeterministic_source_sha"
/usr/bin/cmp -s "${SRC}" "${SRC_REPEAT}" || die "D97ES_nondeterministic_source_bytes"
log "D97ES_SOURCE_SHA256=${SRC_SHA}"
log "D97ES_SOURCE_BYTES=$(/usr/bin/stat -f '%z' "${SRC}")"
log "D97ES_DETERMINISTIC_GENERATION=PASS"
...
/usr/bin/grep -Fq 'D97ESCaptureSlots' "${SRC}" || die "capture_slots_missing"
/usr/bin/grep -Fq 'D97ESCapturedCount' "${SRC}" || die "captured_count_missing"
/usr/bin/grep -Fq 'D97ES01Id' "${SRC}" || die "tuple_id_missing"
/usr/bin/grep -Fq 'D97ES01Mode' "${SRC}" || die "tuple_mode_missing"
/usr/bin/grep -Fq 'D97ES01BadBits' "${SRC}" || die "tuple_badbits_missing"
/usr/bin/grep -Fq 'D97ES01GoodBits' "${SRC}" || die "tuple_goodbits_missing"
/usr/bin/grep -Fq 'D97ES01Ret' "${SRC}" || die "tuple_ret_missing"
/usr/bin/grep -Fq 'atomic_store_explicit(&d97esValid[slot], 1U, memory_order_release)' "${SRC}" || die "tuple_publish_barrier_missing"
/usr/bin/grep -Fq '(!observerRequested || observerTupleCaptured)' "${SRC}" || die "publisher_liveness_gate_missing"
...
log "STATIC_LIVE_TELEMETRY_ONLY_AUDIT=PASS"
...
(cd "${SCAFFOLD}" && /usr/bin/xcodebuild -project FeatureUnlock.xcodeproj -target FeatureUnlock \
 ...
 MODULE_NAME=com.oclpmetalcompat.OCLPMetalCompat MODULE_VERSION=0.0.11 \
 MARKETING_VERSION=0.0.11 CURRENT_PROJECT_VERSION=0.0.11 build) 2>&1 | tee -a "${LOG}"
RC=${PIPESTATUS[0]}; set -e
...
[[ "${VER}" == "0.0.11" ]] || die "version_mismatch"
...
/usr/bin/strings -a "${EXEC}" | /usr/bin/grep -Fq 'D97ELRouteStatus' || die "route_telemetry_marker_missing"
/usr/bin/strings -a "${EXEC}" | /usr/bin/grep -Fq 'D97ES01Mode' || die "tuple_telemetry_marker_missing"
log "BINARY_MARKER_AUDIT=PASS"

cp -R "${KEXT}" "${PACKAGE}/OCLPMetalCompat.kext"
cp "${SRC}" "${PACKAGE}/OCLP7_D97ES_kern_start.cpp"
...
cp "${WORK}/D97ES_generator.py" "${PACKAGE}/OCLP7_D97ES_IOREG_TUPLE_GENERATOR.py"
