...
echo "ACTUAL_SOURCE_BLOB=$ACTUAL_BLOB" | tee -a "$REPORT"
[[ "$ACTUAL_BLOB" == "$EXPECTED_SOURCE_BLOB" ]] || fail "SOURCE_BLOB_MISMATCH"

echo "===== COMPILE =====" | tee -a "$REPORT"
"$CLANG" \
  -std=c++17 \
  -fblocks \
  -O2 \
