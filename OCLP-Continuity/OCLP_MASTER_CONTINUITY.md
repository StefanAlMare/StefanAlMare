...
Source ordering proves this exception occurred at the first D97AF metadata read, before target-byte read, in-memory UUID transform, temporary file, destination sibling or atomic rename. Therefore `D97AF_METHOD_TARGET_MUTATION=NOT_REACHED`. OCLP caught the exception and nevertheless continued to patchset records, Auxiliary Kernel Collection handling and APFS snapshot creation, then printed the generic `Patching complete`. That message is not a D97AF PASS. `D97AF_ROOT_PATCH=INVALID_PARTIAL`; the new inactive snapshot is not authorized for reboot and its exact final MTLCompiler identity remains unread. The deployed D97AF application is runtime-incompatible for this method and must be replaced by a corrected, packaged-runtime-tested build before any further Root Patch attempt.

## CURRENT ACTION — correct packaged-runtime metadata path; no reboot
STOP on the current VESA desktop. Do not reboot into the partial snapshot and do not run Root Patch again from the current D97AF application.
