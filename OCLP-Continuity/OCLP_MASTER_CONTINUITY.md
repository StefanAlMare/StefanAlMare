...
Current authoritative checkpoint: `OCLP-Continuity/checkpoints/OCLP7_CHECKPOINT_20260902_FASTLANE_GITHUB_EXHAUSTIVE_ASUS2_ONLY_FEEDBACK_CONTRACT_D97AEZ_MINIMAL_BUILD.md`
...
## ACTIVE FRONTIER / CURRENT NEXT ACTION — D97AEZ minimal FASTLANE GitHub build/audit; user STOP
D97AEY size-contract repair and its live identity acceptance are closed PASS. Do not repeat the same foreground online/VESA watch, do not induce an unrelated VESA Metal workload, and do not weaken minimum-complete or infer from zero observations. The current online boot is ordinarily the VESA recovery lane under the permanent recovery rule, while the unresolved causal cohort occurs naturally during accelerated boot.

The exact next hardware question is: **during a new failed accelerated boot, do naturally spawned exact-path MTLCompilerService processes map the current D97AD 31-window postimage or different runtime text?** MATCH would make the known P7-vs-D97AD bounded-provenance mismatch hypothesis NEGATIVE only within the 31 tested windows of a completely captured observed accelerated cohort; it would move the inquiry beyond those tested windows without establishing whole-image or runtime-semantic identity. MISMATCH would move the active frontier to deployment/cache/image provenance. No captured natural process or denied task-read remains INCOMPLETE/UNKNOWN. This question directly serves the end goal and changes the repair hypothesis.

The user has accepted that exact test contract and has now fixed the implementation split. The assistant must build and exhaustively test in GitHub the smallest FASTLANE orchestration needed to have the already-audited D97AEX helper active during the later manually authorized accelerated boot. GitHub must cover syntax/static safety, exact identities, state transitions, same-boot skip, first-different-boot one-shot claim, RC0/2/3/4 and contradictory-marker fixtures, interruption/incomplete behavior, report sealing/retrieval and package re-audit. GitHub can prove only those implementation properties; the real runtime result must come only from ASUS2 accelerated hardware.

The eventual ASUS2 sequence must match the established project cycle: one complete identity-pinned arm/deploy wrapper in VESA -> assistant audit -> manual accelerated boot using the already-installed exact D97AD system identity -> bounded wait with no image -> manual hard reset/power-cycle -> VESA recovery -> one pinned retrieval wrapper -> assistant analysis of only that accelerated boot. The user must not compile, combine fragments or run an idle foreground substitute. No ASUS2 command is issued until the complete GitHub lane and artifact audit pass.

No new functional patch and no new Root Patch are presently justified: the installed D97AD file identity is exact and stable. D97AEZ is observation transport for the existing diagnostic, not another compiler patch. No boot orchestration implementation or deployment is yet accepted. Any later orchestration must never launch/stop MTLCompilerService, mutate target/Golden bytes, auto Root Patch or auto reboot, and any collector-owned system mutation must be explicit, bounded, identity-pinned and removable.
