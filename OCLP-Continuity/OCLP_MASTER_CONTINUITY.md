...
Current authoritative checkpoint: `OCLP-Continuity/checkpoints/OCLP7_CHECKPOINT_20260902_D97AEX_VESA_CALIBRATION_RECLASSIFIED_HARDWARE_TEST_CONTRACT_RESET.md`
...
## ACTIVE FRONTIER / CURRENT NEXT ACTION — exact accelerated-hardware test contract before implementation
...
Before writing or building another collector, the assistant must present the minimal test contract and causal decision table to the user. Only then may the GitHub lane design/build/audit the smallest orchestration needed to have the already-audited D97AEX helper active during the later manually authorized accelerated boot. GitHub can prove source/build/package/static safety but cannot claim ASUS2 runtime success. The real runtime result must come only from ASUS2 accelerated hardware, followed by VESA recovery and exact boot-scoped analysis.

No new functional patch and no new Root Patch are presently justified: the installed D97AD file identity is exact and stable. No boot orchestration implementation or deployment is yet accepted. Any later orchestration must never launch/stop MTLCompilerService, mutate target/Golden bytes, auto Root Patch or auto reboot, and any collector-owned system mutation must be explicit, bounded, identity-pinned and removable.
