...
All five standalone Metal segments map successfully; prior sub-page mmap blocker is CLOSED.
Runtime then reaches `mprotect ... to read-write (Metal.PAGE.adhoc)` and exits `RC=-11` SIGSEGV. Target final state is loaded and native system Metal also becomes loaded.
...
Structural transform, parsers, preflight/signing and cold harness PASS.
